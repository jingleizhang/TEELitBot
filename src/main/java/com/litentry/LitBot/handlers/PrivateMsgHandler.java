...
public class PrivateMsgHandler extends ListenerAdapter {
    ...
    public void onMessageReceived(@Nonnull MessageReceivedEvent event) {
        ...
        //log.info("got Private Message, content:{}, author:{}", content, author.getId());
        ...
        //
        //            //check answer
        //            Optional<CaptchaLog> captchaLogOp = captchaLogRepository.findByGuildIdAndDiscordUserId(drop3Guild, author.getIdLong());
        ...
        //                        .setAuthor("Sorry")
        //                        .setDescription(
        //                            "You have 0 attempt remaining to try again. Please restart Captcha Verification by relpy **!start**.\nIf you are not a bot and there has been a mistake, send a message to us in the support channel!\n"
        //                        );
        //                    BotUtils.sendDM(author, embed.build());
        //
        ...
        //
        //                    String desc =
        //                        "Attempt Failed, please try again. You have " + (Constants.MAX_RETRY_TIMES - retryTimes) + " attempts remaining.";
        //                    if (Constants.MAX_RETRY_TIMES == retryTimes) {
        //                        desc = "Attempt Failed. You can reply **!start** or **!resend** to restart.\n";
        //                    }
        //                    final EmbedBuilder embed = EmbedUtils.getDefaultEmbed().setAuthor("Sorry").setDescription(desc);
        //                    BotUtils.sendDM(author, embed.build());
        //                }
        ...
        //            String msg = polkadotVerifyService.getDiscordVerifyMsg(drop3Guild, author.getIdLong());
        //            if (msg == null) {
        //                EmbedBuilder embed = EmbedUtils
        //                    .getDefaultEmbed()
        //                    .setAuthor("Sorry")
        //                    .setDescription("Oops! There is something wrong when I generate Polkadot verify message.");
        //                BotUtils.sendDM(author, embed.build());
        ...
        //
        //                final EmbedBuilder embed = EmbedUtils.getDefaultEmbed().setDescription(desc).setAuthor("Connect Polkadot Wallet");
        //                BotUtils.sendDM(author, embed.build());
        //            } catch (Exception e) {
        //                EmbedBuilder embed = EmbedUtils
        //                    .getDefaultEmbed()
...
//        desc +=
//            "**NOTE**: \n1. The captcha is is Not Case Sensitive and does Not Include Spaces." +
//            " \n2. If it's hard to recognize, you can reply `!resend` command to request a new one.\n\n";
//
//        EmbedBuilder embed = EmbedUtils.embedImage(captcha.getOssURL()).setAuthor("Verify yourself to gain access").setDescription(desc);
//        return embed;
//    }
    ...
    //        }
    //
    //        Optional<DiscordGuildSettings> settingsOP = this.settingsRepository.findByGuildId(guild.getIdLong());
    //        if (settingsOP.isPresent()) {
    //            Long roleId = settingsOP.get().getCaptchaRoleId();
    //            Role role = guild.getRoleById(roleId);
    //            if (role != null) {
    //                log.info("adding Captcha Verified Role {} {} {}", guild.getId(), user.getId(), roleId);
    //                guild
    ...
    //    private void wait(User author, Message sentMsg, Consumer<MessageReactionAddEvent> action) {
    //        waiter.waitForEvent(
