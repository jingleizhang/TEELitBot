package com.litentry.LitBot.command;
...
public abstract class ICommand {
    ...
    public void run(@Nonnull CommandContext ctx) {
        ...
        if (!ctx.getSelfMember().hasPermission(channel, Permission.MESSAGE_SEND)) return;
        ...
        if (this.userPermissions.length > 0 && !ctx.getMember().hasPermission(channel, this.userPermissions)) {
            ...
            ctx.reply(
                "You need the `" +
                parsePerms(this.userPermissions) +
                "` " +
                permissionsWord +
        ...
        if (this.botPermissions.length > 0 && !ctx.getSelfMember().hasPermission(channel, this.botPermissions)) {
            ...
            ctx.reply(
                "I need the `" +
                parsePerms(this.botPermissions) +
                "` " +
                permissionsWord +
        ...
        if (ctx.getArgsJoined().length() > this.maxCharCount) {
            ctx.reply("Maximum character count for arguments cannot exceed `" + this.maxCharCount + "` characters");
            return;
        }
    ...
    public void sendUsageEmbed(TextChannel channel, String prefix, String invoke, String title) {
        MessageEmbed msg = getUsageEmbed(prefix, invoke, title);
        BotUtils.sendEmbed(channel, msg);
    }

    public MessageEmbed getUsageEmbed(String prefix, String invoke, String title) {
        final StringBuilder str = new StringBuilder();

        // Append Usage
        if (this.multilineHelp) str.append(this.usage.replace("{p}", prefix).replace("{i}", invoke)); else str
            .append("**Usage:**\n")
            .append("```css\n")
            .append(prefix)
            .append(invoke)
        ...
        // Append Help
        if (!this.help.equals("")) str.append("\n**Help**: ").append(this.help.replace("{p}", prefix)).append("\n");

        // Append cooldown
        if (this.cooldown > 0) str.append("**Cooldown**: ").append(this.cooldown).append(" seconds\n");

        EmbedBuilder eb = EmbedUtils.getDefaultEmbed().setAuthor(title).setDescription(str.toString());
        return eb.build();
    }
    ...
    //    private void sendCoolDown(MessageReceivedEvent event, int seconds) {
    //        BotUtils.sendEmbed(
    //            event.getChannel(),
    //            EmbedUtils.getDefaultEmbed().setAuthor("Kindly wait `" + MiscUtils.formatTime(seconds) + "` before using this command").build(),
    //            5
    //        );
    //    }
